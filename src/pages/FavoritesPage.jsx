          ...
          <button
            onClick={(e) => {
              ...
              showConfirm({
                ...
                message: `¿Estás seguro de que deseas quitar "${artwork.title}" de tu colección de favoritos?`,
      ...
      <section className="border-b border-border bg-secondary py-16 md:py-24">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            <span className="text-xs uppercase tracking-[0.3em] text-muted-foreground">Mi Colección</span>
            ...
            <p className="text-pretty mx-auto mt-4 max-w-2xl text-muted-foreground">
              {favorites.length > 0
                ? `Has guardado ${favorites.length} ${favorites.length === 1 ? "obra" : "obras"} en tu colección personal.`
                : "Aquí aparecerán las obras que marques como favoritas."}
            </p>
          </div>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          {favorites.length === 0 ? (
            <EmptyState
              ...
              message="Explora la colección y agrega obras a tu lista de favoritos haciendo clic en el corazón."
              showExploreLink
            />
          ) : (
            <>
